FROM node:24-bookworm-slim AS node-build-base
RUN apt-get update \
...
WORKDIR /app
COPY services/kryptotron/requirements.lock.txt /tmp/kryptotron-requirements.lock.txt
RUN /opt/venv/bin/pip install --no-cache-dir -r /tmp/kryptotron-requirements.lock.txt
...
RUN mkdir -p /data \
    ...
    && chmod 700 /data \
    && chmod 755 /usr/local/bin/ocean-entrypoint
EXPOSE 3000
